...
Slim orchestration layer — delegates to:
  ...
  - embedding_service.py for local embeddings
"""
...
from embedding_service import CHROMA_COLLECTION_NAME, embed_query, get_chroma_collection
...
DB_PATH = Path(
    os.getenv(
        "VECTOR_WEALTH_DB_PATH",
        str(Path(__file__).resolve().parent / "vector_wealth_db"),
    )
)
...
FAST_NEWS_MAX_AGE_DAYS = max(
    1,
    int(os.getenv("FAST_NEWS_MAX_AGE_DAYS", "30")),
)

FAST_NEWS_MAX_CANDIDATES = max(
    20,
    int(os.getenv("FAST_NEWS_MAX_CANDIDATES", "120")),
)
...
news_collection = get_chroma_collection()
...
def _build_ticker_terms(ticker: str) -> list[str]:
    ...
    for part in normalized_input.split():
        if (
            len(part) > 2
            and part not in GENERIC_STOCK_TERMS
        ):
            terms.add(part)

    for suffix in TICKER_SUFFIX_SPLITS:
        if (
            normalized_compact.endswith(suffix)
            and len(normalized_compact) > len(suffix) + 1
        ):
            prefix = normalized_compact[
                : -len(suffix)
            ].strip()
    ...
    if (
        is_single_word
        and not alias_input_match
        and normalized_compact not in STOCK_ALIASES
    ):
    ...
    filtered = [
        ...
        and not (
            len(term) <= 4
            and term in GENERIC_STOCK_TERMS
        )
    ]
...
def _build_search_terms_for_ticker(
    ticker: str,
) -> list[str]:
    terms = [ticker]
...
def _fetch_news_ondemand(
    ticker: str,
    limit: int = ONDEMAND_MAX_ARTICLES,
) -> list[dict[str, str]]:
    ...
    full_query = (
        f"({query_string}) AND "
        "(India OR NSE OR BSE OR stock OR shares "
        "OR Sensex OR Nifty)"
    )
    ...
    try:
        ...
        for item in raw_articles:
            ...
            combined_text = (
                f"{title} {merged_description}"
            ).lower()

            is_relevant = any(
                term in combined_text
                for term in match_terms_lower
            )
...
def _retrieve_chroma_news(
    query: str,
    ticker: str,
    limit: int = 5,
) -> list[dict[str, str]]:
    """
    Retrieve recent news from ChromaDB, then rank it
    by ticker/query relevance and recency.

    The old implementation used:

        collection.get(limit=1500)

    which depended on ChromaDB's record ordering.
    Because the collection contains historical records,
    old documents could dominate the retrieved set even
    when newer live documents existed.

    This implementation first asks ChromaDB for records
    inside the configured recent-news window and then
    applies the same ticker/query relevance filtering.
    """
    ...
    cutoff_dt = (
        datetime.now(UTC)
        - timedelta(
            days=FAST_NEWS_MAX_AGE_DAYS
        )
    )

    cutoff_date = cutoff_dt.strftime(
        "%Y-%m-%d"
    )

    try:
        # Both historical and live ingestion store the
        # UI date in the "Date" metadata field as YYYY-MM-DD.
        #
        # Restrict retrieval to the recent window first.
        response = news_collection.get(
            where={
                "Date": {
                    "$gte": cutoff_date,
                }
        ...
        # Compatibility fallback for databases / Chroma
        # versions where the metadata comparison fails.
        #
        # We still enforce the cutoff in Python below.
        try:
            response = news_collection.get(
                limit=max(
                    1500,
    ...
    for idx, _ in enumerate(ids):
        ...
        ticker_tags = _parse_ticker_tags(
            metadata.get("ticker_tags")
        )

        # Always enforce the recency cutoff in Python too.
        # This protects us when the fallback ChromaDB read
        # above is used.
        parsed_date = parse_datetime_text(
            str(
                metadata.get(
                    "published_at",
                    "",
...
def _split_recent_and_historical(
    rows: list[dict[str, str]],
) -> tuple[
    list[dict[str, str]],
    list[dict[str, str]],
    ...
    for row in rows:
        ...
        if (
            parsed_dt is not None
            and parsed_dt >= cutoff_dt
        ):
...
def run_fast_analysis(
    ...
    search_phrase = _extract_search_phrase(
        query
    )
    ...
    chroma_rows = _retrieve_chroma_news(
        query=query,
        ticker=match_key,
        limit=FAST_NEWS_MAX_CANDIDATES,
    )

    local_rows = _retrieve_local_news(
        query=query,
        ticker=match_key,
        limit=FAST_NEWS_MAX_CANDIDATES,
    )

    all_rows = _dedupe_news_rows(
        chroma_rows + local_rows
    )

    # Determine whether we already have recent data
    # BEFORE using the on-demand NewsAPI fallback.
    recent_rows, historical_rows = (
        _split_recent_and_historical(
            all_rows
        )
    )
    ...
    # IMPORTANT:
    #
    # Previously NewsAPI was called only when all_rows
    # was empty. That meant one stale historical article
    # could prevent a fresh lookup.
    #
    # Now we call NewsAPI whenever there is no recent
    # news for the requested ticker.
    if (
        not recent_rows
        and match_key
        and match_key != "MARKET"
    ):
        ondemand_rows = _fetch_news_ondemand(
            match_key,
            limit=ONDEMAND_MAX_ARTICLES,
        )
    ...
    pattern_rows = (
        recent_rows
        + historical_rows
    )[:FAST_NEWS_MAX_CANDIDATES]

    recent_news_count = len(
        recent_rows
    )

    pattern_news_count = len(
        pattern_rows
    )
    ...
    if no_data_at_all:
        stale_reason = (
            f"No news data found for "
            f"'{match_key}'. This stock may not "
            f"have recent coverage in our data sources."
        )
    elif stale_data:
        stale_reason = (
            f"No recent matched headlines in "
            f"last {FAST_NEWS_MAX_AGE_DAYS} days."
        )
    else:
        stale_reason = ""
    ...
    if ondemand_fetched:
        if recent_rows:
            stale_reason = (
                f"Recent live news fetched "
                f"for '{match_key}'."
            )
        elif not stale_reason:
            stale_reason = (
                f"Live news lookup completed for "
                f"'{match_key}', but no recent "
                f"matching headlines were returned."
            )
    ...
    now_sentiment = average_sentiment(
        recent_rows
    )
...
def _enrich_state(
    ...
    state["peers"] = fetch_peer_comparison(
        ticker
    )
...
def get_genai_client() -> genai.Client:
    ...
    if not api_key:
        raise RuntimeError(
            "GOOGLE_API_KEY is missing in root .env."
        )
...
def analyst_advisor_agent(
    ...
    prompt = (
        "You are a financial analyst and "
        "investment advisor for Indian equities.\n\n"
        "Analyze the following news and provide:\n"
        "1. A sentiment score between -1 "
    ...
    response = _call_with_quota_retry(
        ...
        operation_name=(
            "Analysis and decision generation"
        ),
    )
...
vector_wealth_workflow = (
    build_vector_wealth_graph()
)
...
def run_analysis(
    ...
    if not USE_GENAI_ANALYSIS:
        return run_fast_analysis(
            query
        )

    initial_state: VectorWealthState = {
        "query": query
    }

    try:
        result = (
            vector_wealth_workflow.invoke(
                initial_state
            )
        )
